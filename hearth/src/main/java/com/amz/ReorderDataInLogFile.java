 ...
 * In the case of ties (if there are two lines that are identical except for the identifier), the identifier is used to
 ...
 *
 * Note:
 * Identifier consists of only English letters and numbers. The lines with words are not required to match case and the sort
 ...
 *
 * Explanation:
 * Second, fourth. and fifth lines are the lines with words. According to the lexicographical order, the second line will be
 * reordered first in the log file, then fifth, and the fourth comes in the log file. Next, the lines with numbers come in the
 * order in which these lines were in the input.</p>
 */
public class ReorderDataInLogFile {

    public List<String> solution1(int logFileSize, String[] logLines) {
        List<String> letters = new ArrayList<>();
        List<String> nums = new ArrayList<>();
        List<String> reorderedList = new ArrayList();

        for (int i=0; i<logFileSize; i++) {
            String[] words = logLines[i].split(" ", 2);
            if (Character.isDigit(words[1].charAt(0))) {
                nums.add(logLines[i]);
            } else {
                letters.add(words[1].concat(" "+words[0]));
            }
        }
        Collections.sort(letters);
        for (String s : letters) {
            int i = s.lastIndexOf(" ");
            reorderedList.add(s.substring(i+1, s.length()) +" "+ s.substring(0, i));
        }
        reorderedList.addAll(nums);
        return reorderedList;
    }



    boolean onlyLetter(String word) {
